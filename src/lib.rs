...
pub fn run_program_with_test_encode(
    ...
    run::<true>(vm.clone(), storage, &mut []).unwrap_or((ExecutionOutput::Panic, vm))
}

fn run_opcodes(vm: VMState, storage: &mut dyn Storage) -> (ExecutionOutput, VMState) {
    run::<false>(vm.clone(), storage, &mut []).unwrap_or((ExecutionOutput::Panic, vm))
}
...
pub fn run<const UseTestEncode: bool>(
    ...
    loop {
        let opcode = match UseTestEncode {
            true => vm.get_opcode_with_test_encode()?,
            false => vm.get_opcode()?,
        };
