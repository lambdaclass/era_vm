...
impl Precompile for Secp256r1VerifyPrecompile {
    fn execute_precompile(&mut self, query: U256, heaps: &mut Heaps) -> Result<(), EraVmError> {
        ...
        let (hash_value, _) = heaps.read(read_heap_idx, addr)?;
        let (r_value, _) = heaps.read(read_heap_idx, addr + 1)?;
        let (s_value, _) = heaps.read(read_heap_idx, addr + 2)?;
        let (x_value, _) = heaps.read(read_heap_idx, addr + 3)?;
        let (y_value, _) = heaps.read(read_heap_idx, addr + 4)?;
        addr += 4;
