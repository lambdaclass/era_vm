...
pub fn _this(vm: &mut VMState, opcode: &Opcode) {
    address_operands_store(vm, opcode, address_to_u256(&vm.current_frame.this_address));
    return;
}

pub fn _caller(vm: &mut VMState, opcode: &Opcode) {
    address_operands_store(vm, opcode, address_to_u256(&vm.current_frame.msg_sender));
    return;
}

pub fn _code_address(vm: &mut VMState, opcode: &Opcode) {
    address_operands_store(vm, opcode, address_to_u256(&vm.current_frame.code_address));
    return;
}
...
pub fn _ergs_left(vm: &mut VMState, opcode: &Opcode) {
    address_operands_store(vm, opcode, U256::from(vm.current_frame.ergs_remaining));
    return;
}

pub fn _sp(vm: &mut VMState, opcode: &Opcode) {
    let sp = vm.current_frame.stack.sp();
    address_operands_store(vm, opcode, U256::from(sp));
    return;
}

pub fn _get_context_u128(vm: &mut VMState, opcode: &Opcode) {
    address_operands_store(vm, opcode, U256::from(vm.current_frame.context_u128));
    return;
}

pub fn _set_context_u128(vm: &mut VMState, opcode: &Opcode) {
    let (src0, _) = address_operands_read(vm, opcode);
    vm.current_frame.context_u128 = src0.as_u128();
    return;
}
