...
pub fn _heap_read(vm: &mut VMState, opcode: &Opcode) {
    ...
    let gas_cost = vm.current_context_mut().heap.expand_memory(addr + 32); // TODO: Handle ergs cost
    vm.current_context_mut().gas_left -= gas_cost;

    let value = vm.current_context_mut().heap.read(addr);
