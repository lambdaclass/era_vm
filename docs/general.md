...
- When the `far_call` is done.

If the function is non-payable, and `context.get_context_u128` is not zero, the contract will revert. We can see this in the following example:
...
The assembly generated by the contract is the following:
...
In this case, the function is not payable, so, after calling the constructor (block `@.BB0_1`), it checks if the `context.get_context_u128` is set to zero, and if it is not, it means that wei is being passed and it will be reverted (jumping to the `.BB0_2` block) because it is not a `payable` function.

<!-- Show an example of the assembly generated by a function doing a `call` sending money and how it ends up calling the `MsgValueSimulator` (address `0x8009`, `32777` in decimal) which in turn calls the `L2BaseToken` contract. -->

An example of a contract with a payable function in solidity is the following:
...
```
Let's compile this contract and see the assembly generated by the `sendMoney` function:
...
.BB0_14:
	or	@CPI0_5[0], r1, r1
	add	32777, r0, r2 // the MsgValueSimulator contract address
	add	r0, r0, r5
.BB0_13:
