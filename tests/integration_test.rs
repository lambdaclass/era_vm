...
fn test_add_yul() {
    ...
    let (result, _) = run_program_in_memory(&bin_path, Address::default(), Address::default());
...
fn test_add_stack_out_of_bounds() {
    ...
    run_program_in_memory(&bin_path, Address::default(), Address::default());
}
...
fn test_add_runs_if_eq_is_set() {
    ...
    let (result, _final_vm_state) = run_program_with_custom_state(
        &bin_path,
...
fn test_add_sets_overflow_flag() {
    ...
    let (_result, final_vm_state) = run_program_with_custom_state(
        &bin_path,
        vm_with_custom_flags,
        Address::default(),
        Address::default(),
...
fn test_add_sets_eq_flag() {
    ...
    registers[1] = r2;
    let vm_with_custom_flags = VMStateBuilder::new().with_registers(registers).build();
    let (result, final_vm_state) = run_program_with_custom_state(
        &bin_path,
        vm_with_custom_flags,
        Address::default(),
        Address::default(),
...
fn test_mul_asm() {
    ...
    let (_, vm) = run_program_in_memory(&bin_path, Address::default(), Address::default());
...
fn test_mul_big_asm() {
    ...
    let (_, vm) = run_program_with_custom_state(
        &bin_path,
        vm_with_custom_flags,
        Address::default(),
        Address::default(),
...
fn test_mul_conditional_gt_set() {
    ...
    let (result, _) = run_program_with_custom_state(
        &bin_path,
        vm_with_custom_flags,
        Address::default(),
        Address::default(),
...
fn test_vm_generates_frames_and_spends_gas() {
    ...
    let (_, final_vm_state) = run_program(&bin_path, Address::default(), Address::default());
...
fn test_db_storage_add() {
    let bin_path = make_bin_path_asm("add");
    let (result, _) = run_program_with_storage(
        &bin_path,
        "./tests/test_storage".to_string(),
        Address::default(),
        Address::default(),
    );
    assert_eq!(result, U256::from_dec_str("3").unwrap());
...
fn test_heap_store_inc() {
    ...
    registers[1] = r2;
    let vm_with_custom_flags = VMStateBuilder::new().with_registers(registers).build();
    let (result, new_vm) = run_program_with_custom_state(
        &bin_path,
        vm_with_custom_flags,
        Address::default(),
        Address::default(),
...
fn test_near_call() {
    ...
    let (result, vm) = run_program(&bin_path, Address::default(), Address::default());
...
fn test_near_call_stack() {
    ...
    let (result, _) = run_program(&bin_path, Address::default(), Address::default());
    assert_eq!(result, U256::from(5));
}
...
fn test_near_call_eq_flag_restore() {
    ...
    let (_, vm_state) = run_program(&bin_path, Address::default(), Address::default());
...
fn test_shl_asm() {
    ...
    let (_, vm) = run_program(&bin_path, Address::default(), Address::default());
