...
fn test_db_storage_add() {
    let bin_path = make_bin_path_asm("add");
    let db = TestDB::new();
    let vm = VMStateBuilder::new().with_storage(db.ptr.clone()).build();
...
fn test_near_call_callee_uses_gas() {
    ...
    let context = Context::new(program, 5552, address_for_contract); // 1 near call, 1 sstore, 1 add and 2 ret
    let vm = VMStateBuilder::new().with_contexts(vec![context]).build();
...
fn test_heap_read_gas() {
    ...
    let program_code = program_from_file(&bin_path);
    let db = TestDB::new();
    let contexts = Context::new(program_code, 5550, H160::zero());
    let vm = VMStateBuilder::new()
        .with_contexts(vec![contexts])
...
fn test_aux_heap_store_gas() {
    ...
    let program_code = program_from_file(&bin_path);
    let db = TestDB::new();
    let context = Context::new(program_code, 5556, H160::default());
    let vm = VMStateBuilder::new()
        .with_storage(db.ptr.clone())
        .with_contexts(vec![context])
        .build();
